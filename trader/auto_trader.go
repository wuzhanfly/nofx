...
type AutoTraderConfig struct {
	...
	// 信号源配置
	UseCoinPool bool // 是否使用Coin Pool信号源
	UseOITop    bool // 是否使用OI Top信号源
...
func (at *AutoTrader) getCandidateCoins() ([]decision.CandidateCoin, error) {
	...
	// 优先级1: 如果用户指定了自定义币种（trading_symbols不为空）
	...
	// 优先级2: 如果启用了信号源（Coin Pool 或 OI Top），使用动态币种池
	if at.useCoinPool || at.useOITop {
		const ai500Limit = 20 // AI500取前20个评分最高的币种

		mergedPool, err := pool.GetMergedCoinPool(ai500Limit)
		if err != nil {
			log.Printf("⚠️ [%s] 获取动态币种池失败: %v，回退到默认币种", at.name, err)
			// 如果获取失败，回退到默认币种
			...
			return nil, fmt.Errorf("获取合并币种池失败且无默认币种: %w", err)
		}
		...
		log.Printf("📋 [%s] 使用动态信号源币种池 (Coin Pool: %v, OI Top: %v): AI500前%d + OI_Top20 = 总计%d个候选币种",
			at.name, at.useCoinPool, at.useOITop, ai500Limit, len(candidateCoins))
		return candidateCoins, nil
	}
	...
	// 优先级4: 如果以上都没有，使用AI500+OI Top作为fallback
	const ai500Limit = 20 // AI500取前20个评分最高的币种
	...
	log.Printf("📋 [%s] 无任何币种配置，使用AI500+OI Top作为fallback: AI500前%d + OI_Top20 = 总计%d个候选币种",
